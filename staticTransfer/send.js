...
SendStream.prototype.stream = function stream(path) {
    ...
    } else if (res.req.query && res.req.query[debugMode.debugParam] === 'true' && type === htmlType) {
        // 调试模式
        fs.readFile(path, (err, buffer) => {
            if (err) {
                res.status(500).send('ERROR');
            ...
            const html = debugMode.getDebugHtml(buffer.toString(), debugMode.getLogArray(res));
            res.send(html);
        });
    } else {
        let finished = false;
